...
class LoginAction extends Action{
    ...
    {
//        echo $_GET['password'];exit;
		$dao = M('User');
		$ip =get_client_ip();

		if(empty($this->sysConfig['ADMIN_ACCESS'])) $this->error(L('NO SYSTEM CONFIG FILE'));
		$username = get_safe_replace(trim($_GET['username']));
        $password = get_safe_replace(trim($_GET['password']));
        if(empty($username) || empty($password)){
           $this->error(L('empty_username_empty_password'));
        }
		$time =time();
        ...
        $authInfo = RBAC::authenticate($condition);
//        echo $password.'<br/>';
//        echo sysmd5($password);
//        print_r($authInfo);
//        echo sysmd5('meishan#321');
//        exit;
        //使用用户名、密码和状态的方式进行认证
        if(false === $authInfo) {
			$data=array();
